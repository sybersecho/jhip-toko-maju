...
import com.toko.maju.service.util.SessionUtil;
...
public class SaleTransactionsResource {
	 ...
	 *         saleTransactionsDTO, or with status 400 (Bad Request) if the
	 *         saleTransactions has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/sale-transactions")
	public ResponseEntity<SaleTransactionsDTO> createSaleTransactions(
			@Valid @RequestBody SaleTransactionsDTO saleTransactionsDTO) throws Exception {
		log.debug("REST request to save SaleTransactions : {}", saleTransactionsDTO);
		if (saleTransactionsDTO.getId() != null) {
			throw new BadRequestAlertException("A new saleTransactions cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
	 ...
	 *         saleTransactionsDTO, or with status 400 (Bad Request) if the
	 *         saleTransactionsDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the saleTransactionsDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/sale-transactions")
	public ResponseEntity<SaleTransactionsDTO> updateSaleTransactions(
			@Valid @RequestBody SaleTransactionsDTO saleTransactionsDTO) throws Exception {
	...
	public ResponseEntity<List<SaleTransactionsDTO>> getAllSaleTransactions(SaleTransactionsCriteria criteria,
			Pageable pageable) {
	 ...
	 *         saleTransactionsDTO, or with status 404 (Not Found)
	 */
	...
	/**
	 * SEARCH /_search/sale-transactions?query=:query : search for the
	 * saleTransactions corresponding to the query.
	 *
	...
	public ResponseEntity<List<SaleTransactionsDTO>> searchSaleTransactions(@RequestParam String query,
			Pageable pageable) {
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page,
				"/api/_search/sale-transactions");
		return ResponseEntity.ok().headers(headers).body(page.getContent());
	}

	@PostMapping("/sale-transactions/add-to-session")
	public ResponseEntity<SaleTransactionsDTO> addSaleTransactionsToSession(
			@RequestBody SaleTransactionsDTO saleTransactionsDTO, HttpServletRequest request) throws Exception {
		log.debug("REST request to add SaleTransactions : {} to session", saleTransactionsDTO);
//        if (saleTransactionsDTO.getId() != null) {
//            throw new BadRequestAlertException("A new saleTransactions cannot already have an ID", ENTITY_NAME, "idexists");
//        }
		SaleTransactionsDTO result = SessionUtil.addSaleIntoSession(request, saleTransactionsDTO);

		return ResponseEntity.ok().body(result);
				//.headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, result.getCustomerFirstName())).body(result);
	}

    @GetMapping("/sale-transactions/get-in-session")
	public ResponseEntity<SaleTransactionsDTO> getSaleTransactionsInSession( HttpServletRequest request) throws Exception {
		// log.debug("REST request to add SaleTransactions : {} to session", saleTransactionsDTO);
//        if (saleTransactionsDTO.getId() != null) {
//            throw new BadRequestAlertException("A new saleTransactions cannot already have an ID", ENTITY_NAME, "idexists");
//        }
		SaleTransactionsDTO result = SessionUtil.getSaleInSession(request);
